...
function TierPrice({ tier }: { tier: Tier }) {
  if (tier.id === "community") {
    return (
      <div className="mt-6">
        ...
        <p className="mt-1 text-sm text-steel">für immer, self-hosted</p>
      </div>
    );
  }
  ...
  if (tier.id === "enterprise") {
    return (
      <div className="mt-6">
        <span className="text-4xl font-extrabold text-gray-900">Auf Anfrage</span>
        <p className="mt-1 text-sm text-steel">individuelle Staffelpreise</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6">
      <span className="text-sm font-medium text-steel">ab</span>{" "}
      <span className="text-4xl font-extrabold text-gray-900">
        {formatEuro(cheapestYearlyPerEmployee())}
      </span>
      ...
      <p className="mt-1 text-sm text-steel">
        Jahresabo, gestaffelt nach Mitarbeiterzahl
      </p>
    </div>
  );
}
          ...
          >
            ...
            <div className="mt-8">
              ...
              {tier.id === "business" && (
                ...
                >
                  {pending ? "Checkout wird geöffnet …" : "Jahresabo starten"}
                </button>
              )}
              {tier.id === "enterprise" && (
                <a
